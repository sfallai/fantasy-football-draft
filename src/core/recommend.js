...
export function sleepers(pool, ctx, limit = 2) {
  ...
  for (const player of pool) {
    ...
    // A player who qualifies on both is filed under whichever claim is stronger —
    // the same normalised comparison as before — and occupies only that one slot.
    // Ties keep the falling claim, matching the pre-existing tie-break.
    if (fallClaim && (!edgeClaim || fallClaim.strength >= edgeClaim.strength)) {
      fallers.push({ player, rank: fallClaim.strength, why: fallClaim.why });
    } else {
      outliers.push({ player, rank: edgeClaim.strength, why: edgeClaim.why });
    }
  }
  ...
  // is never left empty while any candidate, of either kind, remains.
